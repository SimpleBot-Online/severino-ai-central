...
import { Plus, Search, Cpu, Trash2, Zap, Settings, Timer } from 'lucide-react';
...
const ChipHeating = () => {
  ...
  const filteredInstances = instances.filter(instance => instance.name.toLowerCase().includes(searchTerm.toLowerCase()) || instance.phone.includes(searchTerm)).sort((a, b) => a.name.localeCompare(b.name));
  ...
  return <AppLayout>
      <div className="animate-fadeIn">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
          ...
          <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 w-full md:w-auto mt-4 md:mt-0">
            <div className="relative">
              ...
              <Input placeholder="Buscar chips..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-severino-gray border-severino-lightgray" />
            </div>
            
            <Button variant="outline" className="bg-severino-gray border-severino-lightgray" onClick={() => window.location.href = '/settings'}>
        ...
        {filteredInstances.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredInstances.map(instance => <Card key={instance.id} className="bg-severino-gray border-severino-lightgray hover:border-severino-pink/50 transition-colors">
                ...
                setIsDeleteDialogOpen(true);
                  ...
                  <Button onClick={() => handleHeatChip(instance.id)} className="w-full bg-severino-pink hover:bg-severino-pink/90" disabled={instance.status === 'heating' || instance.status === 'active' || !settings.evolutionApiKey}>
                    {instance.status === 'heating' ? <div className="flex items-center">
                        <div className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full mr-2" />
                        Aquecendo...
                      </div> : instance.status === 'active' ? <>
          ...
          </div> : <div className="flex flex-col items-center justify-center p-12 bg-severino-gray rounded-lg">
            <Cpu size={48} className="text-gray-500 mb-4" />
            <h3 className="text-xl font-semibold mb-2">Nenhuma instância encontrada</h3>
            <p className="text-gray-400 text-center mb-4">
              {searchTerm ? 'Nenhuma instância corresponde à sua busca.' : 'Você ainda não tem nenhuma instância registrada.'}
            </p>
            <Button onClick={() => setIsAddDialogOpen(true)} className="bg-severino-gray">
              <Plus size={18} className="mr-2" />
              Adicionar Instância
        ...
        <DialogContent className="bg-severino-gray border-severino-lightgray sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input placeholder="Ex: Instância Principal, Bot Atendimento" value={newInstance.name} onChange={e => setNewInstance({
              ...newInstance,
              name: e.target.value
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="Ex: +5511999999999" value={newInstance.phone} onChange={e => setNewInstance({
              ...newInstance,
          ...
          <DialogFooter>
            <Button onClick={() => setIsAddDialogOpen(false)} variant="outline" className="bg-severino-lightgray text-white border-severino-lightgray hover:bg-severino-lightgray/80">
              Cancelar
            </Button>
            <Button onClick={handleAddInstance} className="bg-severino-pink hover:bg-severino-pink/90">
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent className="bg-severino-gray border-severino-lightgray sm:max-w-md">
          ...
          {currentInstance && <div className="py-4">
          ...
          <DialogFooter>
            <Button onClick={() => setIsDeleteDialogOpen(false)} variant="outline" className="bg-severino-lightgray text-white border-severino-lightgray hover:bg-severino-lightgray/80">
              Cancelar
            </Button>
